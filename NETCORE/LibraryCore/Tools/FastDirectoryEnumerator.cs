...
{
    //Source https://stackoverflow.com/questions/17756042/improve-the-performance-for-enumerating-files-and-folders-using-net
    //Source wilsone8, CodeProject, https://www.codeproject.com/Articles/38959/A-Faster-Directory-Enumerator
    ...
    public static class FastDirectoryEnumerator
    {
        ...
        private class FileEnumerator : IEnumerator<FileData>
        {
            ...
            private int subDirectoryLimit = 1024;
            ...
            public bool MoveNext()
            {
                ...
                if (m_currentContext.SubdirectoriesToProcess == null)
                {
                    ...
                    {
                        if (subDirectoryCount < subDirectoryLimit)
                        {
                ...
                else if (m_searchOption == SearchOption.AllDirectories)
                {
                    ...
                    if (m_currentContext.SubdirectoriesToProcess == null)
                    {
                        ...
                        try { subDirectories = Directory.EnumerateDirectories(m_path).AsParallel().ToArray(); }
                        catch (UnauthorizedAccessException) { }

                        if (subDirectories != null)
                        { m_currentContext.SubdirectoriesToProcess = new Stack<string>(subDirectories); }
                        else
                        { m_currentContext.SubdirectoriesToProcess = new Stack<string>(); }
                    }
